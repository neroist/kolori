...
{
	float adjustment = 10.;
	float fixed_k = k / 10. * (1. - k);
	float inner = cos(PI*x)*sin(PI*x)*pow(pow(sin(PI*x),2.) + fixed_k, -1./2.);
	return x - (1./PI)*acos(inner) + 1.;
}

// k in [0, 1]
float smooth_ceil_fast(float x, float k)
{
	return floor(x) + pow(x - floor(x), k);
}
...
vec4 color_discrete_slices(vec2 z)
{
	float normalized_phase = (atan(z.y, z.x) + PI) / TAU;
	float num_slices = float(textureSize(slices, 0).x);
	ivec2 texel_coordinate = ivec2(normalized_phase * num_slices, 0);
	return texelFetch(slices, texel_coordinate, 0);
}
...
{
	return texture(image, z * image_aspect_ratio);
}
...
void main()
{
	...
	#elif defined(USE_DISCRETE_SLICES)
		FragColor = color_discrete_slices(f(z));
	#elif defined(USE_CONTINUOUS_SLICES)
		FragColor = color_continuous_slices(f(z));
	#else
